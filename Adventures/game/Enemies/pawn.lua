...
enemy.pawn.type = "pawn"

function enemy.pawn.load()
	pawnpics = {angry = love.graphics.newImage("Art/Enemy Pics/Pawns/joel pawn.png"), scared = love.graphics.newImage("Art/Enemy Pics/Pawns/scared pawn.png"), original = love.graphics.newImage("Art/Enemy Pics/Pawns/original pawn.png")}
	
	pawnpics.squashed = {angry = love.graphics.newImage("Art/Enemy Pics/Pawns/squashed joel pawn.png"), scared = love.graphics.newImage("Art/Enemy Pics/Pawns/squashed scared pawn.png"),original = love.graphics.newImage("Art/Enemy Pics/Pawns/squashed original pawn.png")}
	
	pawnpics.animation = {}
	pawnpics.animation.original = {}
	pawnpics.animation.original.delay = 0.1
	for i=1,6 do
		table.insert(pawnpics.animation.original, love.graphics.newImage("Art/Enemy Pics/Pawns/original animation/" .. i .. ".png"))
	end

	pawnpics.animation.angry = {}
	pawnpics.animation.angry.delay = 0.02
	for i=1,6 do
		table.insert(pawnpics.animation.angry, love.graphics.newImage("Art/Enemy Pics/Pawns/joel animation/" .. i .. ".png"))
	end

	pawnpics.animation.scared = {}
	pawnpics.animation.scared.delay = 0.05
	for i=1,6 do
		table.insert(pawnpics.animation.scared, love.graphics.newImage("Art/Enemy Pics/Pawns/scared animation/" .. i .. ".png"))
	end
...
function enemy.pawn.make(atts)
	...
	currentLevel.numPawns = currentLevel.numPawns + 1
	p.allindex = currentLevel.numPawns

	currentLevel.allpawns[p.allindex] = p
	return p
...
function enemy.pawn.updatePawnCollisions()
	for a=1,currentLevel.numPawns do
		local b = currentLevel.allpawns[a]
		if b ~= nil and not b.killed.type then
			for i=a+1,currentLevel.numPawns do
				v = currentLevel.allpawns[i]
				...
				if v ~= nil and not v.killed.type then
					...
					if collide then
						...
						elseif direction == "top"  then
							...
							b.yspeed = b.yspeed + math.abs(v.yspeed)/2
							v.yspeed = 0
						elseif direction == "bottom"  then
							v.yspeed = v.yspeed + math.abs(b.yspeed)/2
							b.yspeed = 0
...
function enemy.pawn:draw()
	...
	elseif self.killed.type == "arrow" then
		love.graphics.setColor(255,255,255,self.killed.countdown*255/3)
...
function enemy.pawn:getHurt(damage,typ,countdown,hurtatts,deathatts)
...
function enemy.pawn:getCollisionBox()
