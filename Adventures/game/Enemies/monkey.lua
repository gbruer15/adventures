...
enemy.monkey = {}
enemy.monkey.__index = enemy.monkey
enemy.monkey.type = "monkey"

function enemy.monkey.load()
	monkeypics = {}
	monkeypics.pic = love.graphics.newImage("Art/Enemy Pics/Monkeys/monkey.png")
	
	monkeypics.width = monkeypics.pic:getWidth()
	monkeypics.height = monkeypics.pic:getHeight()

end

function enemy.monkey.make(atts)
	...
	setmetatable(p,enemy.monkey)
	p.metatable = 'enemy.monkey'
	...
	p.drawwidth = atts.drawwidth or 90
	p.drawheight = p.drawwidth * monkeypics.height/monkeypics.width
	
	p.xp = atts.xp or math.ceil(p.drawwidth/90)

	p.actualwidth = atts.actualwidth or p.drawwidth
	p.actualheight = atts.actualheight or p.drawheight
	...
	p.leftBoundary = atts.leftBoundary or false
	p.rightBoundary = atts.rightBoundary or false

	p.collidewith = {}
	
	currentLevel.numMonkeys = currentLevel.numMonkeys + 1
	p.allindex = currentLevel.numMonkeys
	
	currentLevel.allmonkeys[p.allindex] = p
	return p
...
function enemy.monkey:update(dt)
	...
	if v.animdelay > monkey.animation[v.pic].delay then
		v.animdelay = 0
		v.anim = v.anim + 1
		if v.anim > 6 then
			v.anim = 1
	...
	--[[
		for i,b in ipairs(level.allmonkeys) do
	...
	if self.yspeed*1.1 >= self.flapPower then
		self:flapWings()
	end
	
	if self.leftBoundary and self.x - self.actualwidth/2 < self.leftBoundary then
		self.xspeed = math.abs(self.xspeed)
	elseif self.rightBoundary and self.x + self.actualwidth/2 > self.rightBoundary then
		self.xspeed = -math.abs(self.xspeed)
	end
...
function enemy.monkey.updatemonkeyCollisions()
	for a=1,currentLevel.nummonkeys do
		local b = currentLevel.allmonkeys[a]
		if b ~= nil and not b.killed.type then
			for i=a+1,currentLevel.nummonkeys do
				v = currentLevel.allmonkeys[i]
...
function enemy.monkey:draw()
	if self.bad then
		love.graphics.setColor(255,255,0)
	end
	--love.graphics.draw(monkeypics.pic,self.x-self.drawwidth/2,self.y-self.drawheight/2,0,self.actualwidth/self.drawwidth,self.actualheight/self.drawheight)
	love.graphics.setColor(50,60,220)
	love.graphics.rectangle("fill",self.x-self.actualwidth/2,self.y-self.actualheight/2,self.actualwidth,self.actualheight)
	
	love.graphics.setColor(20,155,30)
	love.graphics.rectangle("line",self.x-self.actualwidth/2,self.y-self.actualheight/2, self.actualwidth,self.actualheight)
	...
	--love.graphics.rectangle("fill",self:getCollisionBox())
	--love.graphics.rectangle("fill",self.former.x - self.actualwidth/2,self.former.y-self.actualheight/2, self.actualwidth, self.actualheight)
end

function enemy.monkey:die(type,countdown,atts)
	self.health = 0
...
function enemy.monkey:getHurt(damage,typ,countdown,hurtatts,deathatts)
	self.health = self.health - 1
	
	--print("monkey got hurt:")
...
function enemy.monkey:flapWings()
	self.yspeed = -50
end


function enemy.monkey:getCollisionBox()
	return self.x-self.actualwidth/2,self.y-self.actualheight/2, self.actualwidth, self.actualheight
end
