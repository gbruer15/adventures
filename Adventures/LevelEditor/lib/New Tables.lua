...
function tabletostring(list, skiplines, excludelist,notfirst)
	local str="{"
	if skiplines then
		skiplines = "\n"
	else
			...
			if type(v) ~= 'function' and type(v) ~= 'userdata' and type(v) ~= 'thread' then
				if first then
					...
					elseif type(v) == "table" then
						str = str ..  tabletostring(v,false,{},true)
					...
					if type(i) == "string" then
						if type(v) == "table" then
							str = str .. "," .. skiplines .. "['" .. i .. "'] = " ..  tabletostring(v,false,{},true)
						elseif type(v) == "number" or type(v) == 'boolean' or type(v) == 'nil' then
...
function grant.table.save(tab, filename,startstring)

	--local file = love.filesystem.newFile(filename)
	if type(startstring) == 'string' then
		love.filesystem.write( filename, startstring .. tabletostring(tab, true) , all)
	else
